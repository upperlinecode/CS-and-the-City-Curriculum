...
> Note: a component can technically only return a single element, but other elements can be nested inside that one. If you want to return multiple HTML elements, then all of your HTML must be wrapped in a single parent element. The `<div>` in the examples above is a good way to do that.
